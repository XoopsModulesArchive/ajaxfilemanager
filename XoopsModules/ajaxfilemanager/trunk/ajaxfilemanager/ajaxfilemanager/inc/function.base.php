...
function getFolderListing($path,$indexNumber=null, $prefixNumber =' ', $prefixName =' - ',  $outputs=array())
  ...
  * which is calculate from the CONFIG_UPLOAD_VALID_EXTS
...
{
    $validFileExts = explode(',', CONFIG_UPLOAD_VALID_EXTS);
    //exclude those exists in CONFIG_UPLOAD_INVALID_EXTS
    if(CONFIG_UPLOAD_INVALID_EXTS) {
        $invalidFileExts = explode(',', CONFIG_UPLOAD_INVALID_EXTS);
        $validFileExts = array_diff($validFileExts, $invalidFileExts);
    }
    return $validFileExts;
}


 /**
  * get the valid text editor extension 
  * which is calculate from the CONFIG_EDITABALE_VALID_EXTS
  * exclude those specified in CONFIG_UPLOAD_INVALID_EXTS
