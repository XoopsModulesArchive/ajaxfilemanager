 ...
 * @version         $Id: config.ajaxfilemanager.php 6335 2011-03-09 21:27:37Z luciorota $
 */
...
//error_reporting(E_ALL ^ E_NOTICE);	
    ...
    if (!$permAccess) exit(_MA_AJAXFM_NO_PERMISSION);
    ...
    define('CONFIG_OPTIONS_CUT', $permCut);
    $permCopy = ($gpermHandler->checkRight('ajaxfilemanager_extra', 16, $groups, $ajaxfilemanagerModule->mid())) ? true : false ;
...
//    define('CONFIG_THEME_NAME', (CONFIG_QUERY_STRING_ENABLE && !empty($_GET['theme'])?secureFileName($_GET['theme']):'default'));  //change the theme to your custom theme rather than default
    define('CONFIG_THEME_NAME', (CONFIG_QUERY_STRING_ENABLE && !empty($_GET['theme'])?secureFileName($_GET['theme']):'xoops'));  //change the theme to your custom theme rather than default
