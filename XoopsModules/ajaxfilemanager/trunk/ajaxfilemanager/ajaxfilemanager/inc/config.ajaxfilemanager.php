...
//error_reporting(E_ALL ^ E_NOTICE);	
    ...
    define('CONFIG_THICKBOX_MAX_HEIGHT', 500);
        ...
        $path = str_replace('/', '\\', str_replace (DIRECTORY_SEPARATOR . 'inc', '', dirname(__FILE__)));
        $url = explode('?', $_SERVER['REQUEST_URI']);
        $url = str_replace ('/' ,'\\', $url[0]);
        $commonPath = preg_split('/\\\\[^\\\\.]+\.php/', $url);
        $commonPath = $commonPath[0];
        $rootPath = str_replace ($commonPath ,'', $path);
